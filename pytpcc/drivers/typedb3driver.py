...
class Typedb3Driver(AbstractDriver):
    ...
    def loadConfig(self, config):
        ...
        if self.edition in (EDITION.Core, EDITION.Cluster):
            try:
                # NEW driver API (cluster-support-feature-branch)
                from typedb.driver import DriverTlsConfig
                driver_opts = DriverOptions(DriverTlsConfig.disabled())
                self.driver = TypeDB.driver(addresses=f"{self.addr}", credentials=credentials, driver_options=driver_opts)
            except (ImportError, TypeError):
                # OLD driver API (release 3.10.x)
                try:
                    from typedb.driver import driver_options_new
                    native_opts = driver_options_new(False, None)
                    driver_opts = DriverOptions()
                    driver_opts._native_object = native_opts
                except ImportError:
                    driver_opts = DriverOptions()
                self.driver = TypeDB.driver(address=f"{self.addr}", credentials=credentials, driver_options=driver_opts)
